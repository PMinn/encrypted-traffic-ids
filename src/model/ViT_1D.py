...
class ViT1D(nn.Module):
    def __init__(self, seq_len, patch_size, num_classes, dim, depth, heads, mlp_dim, channels = 1 ,dim_head = 64,dropout = 0.5,emb_dropout=0.5):
        ...
        patch_dim = channels * patch_size# 修改patch_dim為patch_size * channels，因為是1D資料
        ...
        self.to_patch_embedding = nn.Sequential(
            ...
            Rearrange('b c (n p) -> b (n 1) (p 1 c)', n=num_patches, p=patch_size), #change the input form to another form
            nn.LayerNorm(patch_size),   #normalize the contents in each patch that range patch_size
        ...
        self.transformer = Transformer(dim, # dimension
