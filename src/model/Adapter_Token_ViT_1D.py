...
class Adapter_Token_ViT_1D(nn.Module):
    def __init__(
        ...
        dropout = 0.5,
        emb_dropout = 0.5,
        # ==== NEW: Adapter Tokens 參數 ====
        flow_feat_dim: int = 24,   # flow 統計特徵維度（標準化後）
        ...
        self.patch_dim = 1 * patch_size # 修改patch_dim為patch_size * channels，因為是1D資料
        ...
        self.to_patch_embedding = nn.Sequential(
            Rearrange('b c (n p) -> b n (p c)', p = self.patch_size), #change the input form to another form
            nn.LayerNorm(self.patch_dim),   #normalize the contents in each patch that range patch_size
            nn.Linear(self.patch_dim, self.dim), #project each patch to the dimension of dim
            nn.LayerNorm(self.dim) #the form is changed to be 'b n d'
        )

        # ==== NEW: flow_feat -> Adapter Tokens (B,F)->(B,K,D) ====
    ...
    def forward(self, x, flow_feat):
        ...
        if self.num_flow_tokens > 0:
            if flow_feat is None:
                # 安全兜底：若未提供 flow_feat，使用零向量（不建議長期這樣用）
                flow_feat = torch.zeros(batch, self.flow_feat_dim, dtype = x.dtype, device = x.device)
            flow_tokens = self.flow_proj(flow_feat).view(batch, self.num_flow_tokens, self.dim) # [B, K, D]
