...
from src.utils.alias import a2p
...
def split_to_flows_from_file(input_file: Path, output_dir: Path, max_files: int = 50, splitCapPath: Path = a2p("@/src/data_processing/SplitCap.exe"), remove_original: bool = False) -> list | None:
    ...
    # 確認所有檔案都已處理
    if not input_file.exists():
        logging.getLogger("split_to_flows.from_file").info("All files have been processed successfully.")
        return None
    else:
        logging.getLogger("split_to_flows.from_file").warning(f"Some files were not processed: {input_file}")
        return [input_file]
