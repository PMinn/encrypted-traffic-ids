...
def run_training() -> tuple[int, float, float, dict[int, float], dict[int, float]]:
    ...
    result_folder_path: Path = a2p(
        "@/data/CIC-IDS-2017/result/raw_multi/"
    ...
    for epoch in range(1, EPOCH + 1):
        ...
        if (epoch - 1) % 5 == 0:
            torch.save(
                model,
                f"{result_folder_path}/pth/model-{val_acc:.2f}-val_acc-{epoch}-epoch.pth",
            )
...
def evaluate(
    ...
    if not isinstance(f1_per_class_arr, (npt.NDArray, list)):
        raise TypeError(
            f"Expected f1_per_class_arr to be npt.NDArray or list, but got {type(f1_per_class_arr)}"
        )
