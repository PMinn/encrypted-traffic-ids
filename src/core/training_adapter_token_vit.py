...
def run_adapter_token_vit_training(
    ...
    for epoch in range(1, EPOCH + 1):
        ...
        if (epoch - 1) % 5 == 0:
            torch.save(
                model.state_dict(),
                "{}/pt/model-{:.2f}-val_acc-{}-epoch.pt".format(
    ...
    logging.getLogger("run_training").info(
        f"Best model save to: {result_folder_path}/pt/model-best_val_acc.pt"
    )
...
def run_adapter_token_vit_training_with_mlflow(
    experiment_name: str,
    run_name: str,
    config: ConfigDict,
    features: list[str],
    ...
    if config["seed"] is not None:
        set_seed_to(seed=config["seed"])  # 設定隨機種子以確保可重現 (可選)
