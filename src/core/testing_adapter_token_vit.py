...
def run_eval(
    ...
    normalizer_path: Path | None = None,  # 可選：你的 normalizer 檔
    ...
    if normalizer_path is not None:
        normalize = np.load(normalizer_path)
        ...
        mlflow.artifacts.download_artifacts(
            artifact_uri=f"runs:/{mlflow_run_id}/normalize.npz",
            dst_path=str(dst_path),
        )
        normalize = np.load(dst_path / "normalize.npz")
    else:
        raise ValueError("Either normalizer_path or mlflow_run_id must be provided.")
...
def run_adapter_token_vit_testing_with_mlflow(
    run_id: str,
    config: ConfigDict,
    features: list[str],
    dataset_path: Path,  # 可選：你的訓練資料檔
