...
def get_features() -> None:
    ...
    packet_shape = (96, 8)
    pcaps = a2p("@data/data/CIC23/split/Benign").glob("**/*.pcap")
    flow_to_features_file(
        pcaps,
        output_file=a2p(
            f"@data/data/CIC23/features_sspp/features/benign_features_{packet_shape[0]}_{packet_shape[1]}.npy"
        ),
        packet_shape=packet_shape,
    )

    malicious_folders = a2p("@data/data/CIC23/split/Malicious").iterdir()
    skip_folders: list[Path] = []
    for malicious_folder in malicious_folders:
        folder_name = malicious_folder.stem
        ...
        flow_to_features_file(
            pcaps,
            output_file=a2p(
                f"@data/data/CIC23/features_sspp/features/{folder_name}_features_{packet_shape[0]}_{packet_shape[1]}.npy"
            ),
            packet_shape=packet_shape,
        )
...
def split_features_to_train_test(features_folder: str) -> None:
    ...
    train_folder = a2p(f"@data/data/CIC23/{features_folder}/split/train")
    test_folder = a2p(f"@data/data/CIC23/{features_folder}/split/test")
    train_folder.mkdir(parents=True, exist_ok=True)
    test_folder.mkdir(parents=True, exist_ok=True)
    features_npys = a2p(f"@data/data/CIC23/{features_folder}/merged_features").glob(
        "*.npy"
    )
    ...
    with open(
        a2p(f"@data/data/CIC23/{features_folder}/split/train_test_split_info.json"),
        "w",
    ) as f:
...
def run_sampling_and_labeling(features_folder: str, alpha: float) -> None:
    ...
    sampling(
        [
            {
                "name": "benign",
                "data_path": a2p(
                    f"@data/data/CIC23/{features_folder}/split/train/benign_features_96_8.npy"
                ),
            ...
            },
            {
                "name": "Brute_Force",
                "data_path": a2p(
                    f"@data/data/CIC23/{features_folder}/split/train/Brute_Force_features_96_8.npy"
                ),
                "sampling_count": sampling_suggestion["number_of_samples_suggested"][
                    "Brute_Force"
                ],
            },
            {
                "name": "DDoS",
                "data_path": a2p(
                    f"@data/data/CIC23/{features_folder}/split/train/DDoS_features_96_8.npy"
                ),
                "sampling_count": sampling_suggestion["number_of_samples_suggested"][
                    "DDoS"
                ],
            },
            {
                "name": "DoS",
                "data_path": a2p(
                    f"@data/data/CIC23/{features_folder}/split/train/DoS_features_96_8.npy"
                ),
                "sampling_count": sampling_suggestion["number_of_samples_suggested"][
                    "DoS"
                ],
            },
            {
                "name": "Mirai",
                "data_path": a2p(
                    f"@data/data/CIC23/{features_folder}/split/train/Mirai_features_96_8.npy"
                ),
                "sampling_count": sampling_suggestion["number_of_samples_suggested"][
                    "Mirai"
                ],
            },
            {
                "name": "Recon",
                "data_path": a2p(
                    f"@data/data/CIC23/{features_folder}/split/train/Recon_features_96_8.npy"
                ),
                "sampling_count": sampling_suggestion["number_of_samples_suggested"][
                    "Recon"
                ],
            },
            {
                "name": "Spoofing",
                "data_path": a2p(
                    f"@data/data/CIC23/{features_folder}/split/train/Spoofing_features_96_8.npy"
                ),
                "sampling_count": sampling_suggestion["number_of_samples_suggested"][
                    "Spoofing"
                ],
            },
            {
                "name": "Web_based",
                "data_path": a2p(
                    f"@data/data/CIC23/{features_folder}/split/train/Web_based_features_96_8.npy"
                ),
                "sampling_count": sampling_suggestion["number_of_samples_suggested"][
                    "Web_based"
                ],
        ...
        save_to=a2p(
            f"@data/data/CIC23/{features_folder}/sampled_{int(alpha * 100)}P/train/"
        ),
        ...
        [
            {
                "name": "Benign",
                "data_path": a2p(
                    f"@data/data/CIC23/{features_folder}/split/test/benign_features_96_8.npy"
                ),
            },
            {
                "name": "Brute_Force",
                "data_path": a2p(
                    f"@data/data/CIC23/{features_folder}/split/test/Brute_Force_features_96_8.npy"
                ),
            },
            {
                "name": "DDoS",
                "data_path": a2p(
                    f"@data/data/CIC23/{features_folder}/split/test/DDoS_features_96_8.npy"
                ),
            },
            {
                "name": "DoS",
                "data_path": a2p(
                    f"@data/data/CIC23/{features_folder}/split/test/DoS_features_96_8.npy"
                ),
            },
            {
                "name": "Mirai",
                "data_path": a2p(
                    f"@data/data/CIC23/{features_folder}/split/test/Mirai_features_96_8.npy"
                ),
            },
            {
                "name": "Recon",
                "data_path": a2p(
                    f"@data/data/CIC23/{features_folder}/split/test/Recon_features_96_8.npy"
                ),
            },
            {
                "name": "Spoofing",
                "data_path": a2p(
                    f"@data/data/CIC23/{features_folder}/split/test/Spoofing_features_96_8.npy"
                ),
            },
            {
                "name": "Web_based",
                "data_path": a2p(
                    f"@data/data/CIC23/{features_folder}/split/test/Web_based_features_96_8.npy"
                ),
            },
        ],
        save_to=a2p(
            f"@data/data/CIC23/{features_folder}/sampled_{int(alpha * 100)}P/test/"
        ),
    )
...
def get_suggestion(features_folder: str, alpha: float) -> None:
    ...
    suggester(
        {
            "benign_features_96_8": 33164,
            "Mirai_features_96_8": 44951,
            "Brute_Force_features_96_8": 942,
            "Web_based_features_96_8": 28009,
            "Spoofing_features_96_8": 10920,
            "Recon_features_96_8": 14677,
            "DoS_features_96_8": 92769,
            "DDoS_features_96_8": 184843,
        },
        alpha=alpha,
        json_path=a2p(
            f"@data/data/CIC23/{features_folder}/sampled_{int(alpha * 100)}P/sampling_suggestion.json"
        ),
    )
...
def merge_features() -> None:
    ...
    features_folder = a2p("@data/data/CIC23/features_sspp/features")
    output_folder = a2p("@data/data/CIC23/features_sspp/merged_features")
    output_folder.mkdir(parents=True, exist_ok=True)
    feature_files = features_folder.glob("*.npy")
    merge_data_dict = {
        "benign": None,
        "DDoS": None,
        "Spoofing": None,
    ...
    for feature_file in feature_files:
        ...
        if "benign" in filename:
            count["benign"] += 1
            merge_data_dict["benign"] = data
        elif "DDoS" in filename:
            count["DDoS"] += 1
            if merge_data_dict["DDoS"] is None:
                merge_data_dict["DDoS"] = data
            else:
                merge_data_dict["DDoS"] = np.concatenate(
                    [merge_data_dict["DDoS"], data], axis=0
                )
        elif "Spoofing" in filename:
            count["Spoofing"] += 1
            if merge_data_dict["Spoofing"] is None:
                merge_data_dict["Spoofing"] = data
            else:
                merge_data_dict["Spoofing"] = np.concatenate(
                    [merge_data_dict["Spoofing"], data], axis=0
                )
        elif "Mirai" in filename:
            count["Mirai"] += 1
            if merge_data_dict["Mirai"] is None:
                merge_data_dict["Mirai"] = data
            else:
                merge_data_dict["Mirai"] = np.concatenate(
                    [merge_data_dict["Mirai"], data], axis=0
                )
        elif "DictionaryBruteForce" in filename:
            count["Brute_Force"] += 1
            if merge_data_dict["Brute_Force"] is None:
                merge_data_dict["Brute_Force"] = data
            else:
                merge_data_dict["Brute_Force"] = np.concatenate(
                    [merge_data_dict["Brute_Force"], data], axis=0
                )
        elif "DoS" in filename:
            count["DoS"] += 1
            if merge_data_dict["DoS"] is None:
                merge_data_dict["DoS"] = data
            else:
                merge_data_dict["DoS"] = np.concatenate(
                    [merge_data_dict["DoS"], data], axis=0
                )
        elif "Recon" in filename or "Vulnerability" in filename:
            count["Recon"] += 1
            if merge_data_dict["Recon"] is None:
                merge_data_dict["Recon"] = data
            else:
                merge_data_dict["Recon"] = np.concatenate(
                    [merge_data_dict["Recon"], data], axis=0
                )
        else:
            count["Web_based"] += 1
            if merge_data_dict["Web_based"] is None:
                merge_data_dict["Web_based"] = data
            else:
                merge_data_dict["Web_based"] = np.concatenate(
                    [merge_data_dict["Web_based"], data], axis=0
                )
    print(count)
    for key in merge_data_dict:
        np.save(
            output_folder / f"{key}_features_96_8.npy",
            merge_data_dict[key],
            allow_pickle=False,
        )
...
def main() -> None:
    ...
    # 計算出各類型的數量
    # count_pcaps(a2p("@data/data/CIC23/split/"))
