    ...
    def validation(self, model, epoch):
        ...
        with torch.no_grad():
            for inputs, labels in self.valid_loader:
                ...
                outputs, feat = model(inputs)                          # (B, num_classes)
...
class Train_Cnn(Train_1D):
    def __init__(self, model, data_path, label_path, subdir, hyper_parameter, classes):
        super().__init__(model, data_path, label_path, subdir, hyper_parameter, classes)
    ...
    def train(self,):
        ...
        for epoch in range(self.EPOCH):
            ...
            with open(self.subdir+'/train_Acc_Loss.txt', 'a') as file:
                ...
                for i, c in enumerate(self.classes):
                    try:
                        train_acc_add += train_class_correct[i] / train_class_total[i]
                    except ZeroDivisionError:
                        train_acc_add += 0
                    try:
                        acc = 100 * train_class_correct[i] / train_class_total[i]
