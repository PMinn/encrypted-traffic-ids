...
#######################################################################  binary  #############################################################################
    ...
    def validation(self, model, epoch, loss):
        ...
        class_correct = list(0. for _ in self.classes)
        class_total = list(0. for _ in self.classes)
        ...
        with torch.no_grad():
            ...
            overall_f1 = f1_score(all_labels, all_predictions, average='weighted', zero_division=0)

            with open(os.path.join(self.subdir, 'valid_Acc_Loss.txt'), 'a') as file:
                ...
                for i, c in enumerate(self.classes):
                    ...
                    if i < len(precision_per_class):
                        ...
                        file.write(f'Precision: 0.0000 | Recall: 0.0000 | F1 Score: 0.0000\n')
                ...
                file.write(f'F1 Score: {overall_f1:.4f}\n')
                print(f'Valid Accuracy:{val_acc:.4f}| loss:{val_loss:.4f}| F1 Score: {overall_f1:.4f}')

        self.valL_loss.append(val_loss)
        save_dir = os.path.abspath(os.path.join(self.subdir, 'pth'))
        if not os.path.exists(save_dir):
            os.makedirs(save_dir, exist_ok=True)

        if epoch % 5 == 0:
            model_path = os.path.join(save_dir, f'model-{val_acc:.2f}-val_acc-{epoch}-epoch.pth')
            torch.save(model.module if isinstance(model, torch.nn.DataParallel) else model, model_path)

        if val_acc >= self.best_val_acc:
            self.best_val_acc = val_acc
            with open(os.path.join(self.subdir, 'Best_valid_Acc.txt'), 'w') as file:
                file.write('Best Epoch :' + str(epoch + 1) + f'\n Best acc: {self.best_val_acc}')
            best_model_path = os.path.join(save_dir, 'model-best_val_acc.pth')
            torch.save(model.module if isinstance(model, torch.nn.DataParallel) else model, best_model_path)
